...
validate_local_env() {
  ...
  APPLICATION_OTEL_COLLECTOR_ENABLED="${APPLICATION_OTEL_COLLECTOR_ENABLED:-true}"; APPLICATION_OTEL_COLLECTOR_DIR="${APPLICATION_OTEL_COLLECTOR_DIR:-/data/signoz-integrations/application-otel-collector}"; APPLICATION_OTEL_COLLECTOR_IMAGE="${APPLICATION_OTEL_COLLECTOR_IMAGE:-otel/opentelemetry-collector-contrib:0.139.0}"; APPLICATION_OTEL_COLLECTOR_CONTAINER="${APPLICATION_OTEL_COLLECTOR_CONTAINER:-espresso-application-otel-collector}"; APPLICATION_OTEL_COLLECTOR_COOLIFY_NETWORK="${APPLICATION_OTEL_COLLECTOR_COOLIFY_NETWORK:-coolify}"; APPLICATION_OTEL_COLLECTOR_SIGNOZ_NETWORK="${APPLICATION_OTEL_COLLECTOR_SIGNOZ_NETWORK:-signoz-network}"; APPLICATION_OTEL_COLLECTOR_SIGNOZ_ENDPOINT="${APPLICATION_OTEL_COLLECTOR_SIGNOZ_ENDPOINT:-signoz-ingester:4317}"; APPLICATION_OTEL_SERVICE_NAME="${APPLICATION_OTEL_SERVICE_NAME:-espresso}"; APPLICATION_OTEL_ENVIRONMENT="${APPLICATION_OTEL_ENVIRONMENT:-production}"
  [[ "$POSTGRES_COLLECTOR_POSTGRES_PORT" =~ ^[0-9]+$ ]] || die "POSTGRES_COLLECTOR_POSTGRES_PORT deve ser numérica"
  [[ "$APPLICATION_OTEL_ENVIRONMENT" == production ]] || die "APPLICATION_OTEL_ENVIRONMENT deve ser production"
}
...
main() {
  ...
  case "$action" in
    ...
    foundryctl|signoz|postgres-collector|application-otel-collector|signoz-firewall|observability-status|observability-destroy) run_remote_script "$action" ;;
